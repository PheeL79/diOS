...
void SystemInit(void)
{
    ...
    PWR_DeInit();
    PWR_PVDLevelConfig(PWR_LEVEL);
    PWR_PVDCmd(ENABLE);
    //Замечание: Во время режима ожидания PVD отключен, поэтому бит PVDO
    //           читается как 0 после выхода устройства из режима ожидания и
    //           после сброса, пока не установлен бит PVDE.
    while (RESET != PWR_GetFlagStatus(PWR_FLAG_PVDO)) {};
#ifdef USE_IWDT
