...
const S8 freq_idx = FrequencyIdxGet(drv_args_p->info.sample_rate);
    ...
    /* Init the I2S */
    if (HAL_OK != HAL_I2S_Init(&i2s_hd)) { return S_HARDWARE_ERROR; }
...
Status CS4344_DeInit(void* args_p)
{
    ...
    if (HAL_OK != HAL_I2S_DeInit(&i2s_hd)) { return S_HARDWARE_ERROR; }
    /*##-1- Reset peripherals ##################################################*/
...
{
CS4344_DrvArgsInit* drv_args_p = (CS4344_DrvArgsInit*)args_p;
Status s = S_UNDEF;
