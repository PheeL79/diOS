...
Status s = S_UNDEF;
    IF_OK(s = drv_ks8721bl.IoCtl(DRV_REQ_KS8721BL_PHY_RESET, OS_NULL)) {
//        /* Restart the auto-negotiation */
//        IF_OK(s = drv_ks8721bl.IoCtl(DRV_REQ_KS8721BL_AUTO_NEG_RESTART, OS_NULL)) {
//        }
        /* Get tick */
        tick_start = HAL_GetTick();
        /* Wait until the auto-negotiation will be completed */
        do {
            if (HAL_OK != HAL_ETH_ReadPHYRegister(&eth0_hd, REG_01H_BASIC_STATUS, &reg_value)) {
                s = S_TIMEOUT;
                return s;
            }
...
{
Status s = S_UNDEF;
    IF_OK(s = drv_ks8721bl.IoCtl(DRV_REQ_KS8721BL_LINK_INT_SETUP, OS_NULL)) {}
    return s;
}
...
Status s = S_UNDEF;
    switch (request_id) {
        ...
        case DRV_REQ_KS8721BL_LINK_INT_CLEAR:
            ...
            if (HAL_OK == HAL_ETH_ReadPHYRegister(&eth0_hd, REG_1BH_INT_CTRL_STATUS, &reg_value)) {
                s = S_OK;
            } else {
                s = S_TIMEOUT;
            }
        ...
        case DRV_REQ_KS8721BL_LINK_INT_SETUP:
            ...
            if (HAL_OK == HAL_ETH_ReadPHYRegister(&eth0_hd, REG_1BH_INT_CTRL_STATUS, &reg_value)) {
                ...
                if (HAL_OK == HAL_ETH_WritePHYRegister(&eth0_hd, REG_1BH_INT_CTRL_STATUS, reg_value)) {
                    s = S_OK;
                } else {
                    s = S_TIMEOUT;
                }
                ...
                if (ETH_AUTONEGOTIATION_DISABLE != eth0_hd.Init.AutoNegotiation) {
                    if (BIT_TEST(reg_value, BIT(REG_01H_BIT_AUTO_NEG_ABILITY))) {
                        ...
                        if (HAL_OK == HAL_ETH_ReadPHYRegister(&eth0_hd, REG_00H_BASIC_CTRL, &reg_value)) {
                            ...
                            if (HAL_OK == HAL_ETH_WritePHYRegister(&eth0_hd, REG_00H_BASIC_CTRL, reg_value)) {
                                /* Get tick */
                                tick_start = HAL_GetTick();
                                /* Wait until the auto-negotiation will be completed */
                                do {
...
error:
        ...
        case DRV_REQ_KS8721BL_PHY_ID_TEST:
            ...
            U32 phy_id_hi;
            U32 phy_id_lo;
            if (HAL_OK == HAL_ETH_ReadPHYRegister(&eth0_hd, REG_02H_PHY_ID1, &phy_id_hi)) {     // 0x0022
                if (HAL_OK == HAL_ETH_ReadPHYRegister(&eth0_hd, REG_03H_PHY_ID2, &phy_id_lo)) { // 0x1619
                    if ((0x0022 == phy_id_hi) &&
                        (0x1619 == phy_id_lo)) {
                            s = S_OK;
                    } else {
